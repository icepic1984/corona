...
days <- generate_days(as.POSIXct("2020-01-22"),51)
...
dfhop_confirmed <- calculate_total(dfhop_confirmed,days, "Mainland China", "China Total")
...
dfhop_death <- calculate_total(dfhop_death,days, "Mainland China", "China Total")
...
dfhop_recovered <- calculate_total(dfhop_recovered,days, "Mainland China", "China Total")
...
dfhop <- rbind(
    ...
    calculate_growth(dfhop,"US Total"),
     # calculate_growth(dfhop,"Republic of Korea"),
...
#moving average
dfhop_mean <- dfhop %>% filter(Country == "Italy") %>%
